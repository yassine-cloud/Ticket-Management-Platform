...
export class AuthService {
    ...
    async login(dto: LoginDto, context?: AuthRequestContext) {
        ...
        if (existingSession) {
            const deviceInfoForDb = context?.userAgent
                ...
                : (existingSession.deviceInfo ?? undefined) as unknown as Prisma.InputJsonValue | Prisma.NullableJsonNullValueInput;
    ...
    async refresh(dto: RefreshTokenDto, context?: AuthRequestContext) {
        ...
        const deviceInfoForDb = context?.userAgent
            ? ({ userAgent: context.userAgent } as unknown as Prisma.InputJsonValue)
            : (session.deviceInfo ?? undefined) as unknown as Prisma.InputJsonValue | Prisma.NullableJsonNullValueInput;
    ...
    private getAccessTtl(): JwtSignOptions['expiresIn'] {
        return (this.configService.get<string>('JWT_ACCESS_TTL') ?? '15m') as
            JwtSignOptions['expiresIn'];
    }

    private getRefreshTtl(): JwtSignOptions['expiresIn'] {
        return (this.configService.get<string>('JWT_REFRESH_TTL') ?? '7d') as
            JwtSignOptions['expiresIn'];
    }
